...
class Login extends Controlador
{
	...
	public function cambiarClaveCliente(string $id=''):void
	{
		$id=Helper::desencriptar($id);
		$errores=[];
		if ($_SERVER['REQUEST_METHOD']=="POST") {
			...
			if (count($errores)==0) {
				// Nota: para clientes la clave se almacena en texto plano según el diseño actual
				$data = ["clave"=>$clave1, "id"=>$id];
				if ($this->modelo->actualizarClaveCliente($data)) {
					$this->mensaje(
